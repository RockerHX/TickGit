...
<div class="flex h-full min-h-0 flex-col overflow-hidden bg-tg-bg-panel">
  <div class="border-b border-tg-border-soft px-3 py-2.5">
    <div class="flex items-center justify-between gap-3">
      ...
      {#if activeFilterCount > 0}
        <button
          type="button"
          class="tg-control tg-focus-ring px-2 py-0.5 text-[10px] font-medium"
    ...
    <div class="mt-0.5 text-[11px] text-tg-text-secondary/80">
  ...
  <div class="min-h-0 flex-1 overflow-y-auto">
    {#if commits.length === 0 && !loading}
      <div
        class="m-3 rounded-tg-control border border-dashed border-tg-border-strong bg-tg-bg-app px-4 py-8 text-center text-[13px] text-tg-text-muted"
      >
        {activeFilterCount > 0
    ...
    <div class="space-y-2 p-2.5">
      {#each commits as commit (commit.hash)}
        ...
        <button
          type="button"
          class={`tg-focus-ring group relative min-h-[78px] w-full overflow-hidden rounded-xl border px-3 py-2.5 text-left shadow-sm transition duration-150 ${
            isSelected
        ...
        >
          {#if isSelected}
            <div
              class="absolute inset-y-1.5 left-0 w-1 rounded-r-full bg-gradient-to-b from-sky-300 via-tg-blue-soft to-tg-blue shadow-[0_0_14px_rgba(96,165,250,0.55)]"
            ></div>
          {/if}
          ...
          <div class="flex items-start gap-2.5">
            <div class="relative mt-0.5 shrink-0">
              {#if !commit.isPushed}
                {#if commit.isSafePushTarget}
                  <span
                    class="absolute -left-1 -top-1 z-10 flex h-4.5 w-4.5 items-center justify-center rounded-full border border-emerald-300/60 bg-gradient-to-br from-emerald-500/25 to-tg-bg-app text-emerald-200 shadow-[0_0_12px_rgba(52,211,153,0.3)] ring-2 ring-tg-bg-app"
                ...
                {:else}
                  <span
                    class="absolute -left-1 -top-1 z-10 flex h-4.5 w-4.5 items-center justify-center rounded-full border border-rose-300/60 bg-gradient-to-br from-rose-500/25 to-tg-bg-app text-rose-200 shadow-[0_0_12px_rgba(251,113,133,0.26)] ring-2 ring-tg-bg-app"
                    title={pushStatusTitle}
                    aria-label={pushStatusTitle}
                  >
                    <svg
              ...
              <div
                class={`flex h-8 w-8 items-center justify-center rounded-full border text-[10px] font-semibold shadow-sm shadow-black/20 transition ${
                  isSelected
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0 flex-1">
                  <div class="flex min-w-0 items-center gap-1.5">
                    <div
                      class="min-w-0 flex-1 truncate text-[13px] font-semibold leading-5 text-tg-text-primary"
                  ...
                  {#if commit.tags.length > 0}
                    ...
                    >
                      {#each commit.tags as tag}
                        <span
                          class="max-w-[9rem] truncate rounded-full border border-amber-400/30 bg-amber-400/10 px-1.5 py-0.5 text-[9px] font-medium text-amber-200"
                          title={tag}
                        >
                          {tag}
                        </span>
                ...
                <div class="mt-0.5 flex shrink-0 items-center gap-2">
                  ...
                  {#if !commit.isPushed}
                    <span
                      class={`h-2 w-2 rounded-full ring-2 ring-tg-bg-app/90 ${
                        commit.isSafePushTarget
                  ...
                  {:else}
                    <span
                      class="h-2 w-2 rounded-full bg-tg-text-muted ring-2 ring-tg-bg-app/90"
                      title={pushStatusTitle}
                      aria-label={pushStatusTitle}
                    ></span>
                  {/if}
              ...
              <div
                class="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] text-tg-text-secondary/80"
              >
    ...
    {#if loading}
      <div class="px-3 py-3 text-center text-[11px] text-tg-text-secondary/80">
        {translate($locale, "history.loading")}
      </div>
    {/if}
  </div>

  <div class="border-t border-tg-border-soft bg-tg-bg-card px-3 py-2.5">
    <div class="text-[11px] text-tg-text-secondary/80">
    ...
    {#if pagination.totalPages > 1}
      <div class="mt-2 flex items-center gap-1.5">
        <button
          type="button"
          class="tg-control tg-focus-ring flex h-7 w-7 items-center justify-center border-transparent disabled:cursor-not-allowed disabled:opacity-40"
        ...
        {#each pagination.buttons as button}
          ...
          {:else}
            <button
              type="button"
              class={`h-7 min-w-7 rounded-md px-2 text-xs font-semibold transition ${
                button.active
