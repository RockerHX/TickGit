...
- 保留现有功能能力，不在本轮 UI 改造中新增 Git 行为语义。
...
- Tabs 改为更轻的 pill/segmented 样式，选中态使用蓝色渐变或蓝色底色。
...
- 不与 Changes 模式中的 commit 输入框混淆。
- History 模式只读；Changes 模式仍保留可编辑 commit message 和提交按钮。
...
- `WorkspaceChangesPanel.svelte`
  - 视觉跟随 Changed Files / Diff 新样式，但保留工作区 Git 行为。
- `DiffViewer.svelte`
...
6. 保持 Changes 模式 commit 输入区域逻辑不变，仅做视觉协调。
